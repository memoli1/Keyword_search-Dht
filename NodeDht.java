...
public class NodeDht extends Thread implements Comparable<NodeDht> {
	...
	private boolean iAmResponsibleForId (int obj_id){
		if (obj_id > predecessor.getmyId() && obj_id <= myid) {
			return true;
		}
		else if (myid < predecessor.getmyId() && (obj_id <= myid || obj_id > predecessor.getmyId())){
			return true;
		}
		else {
			return false;
	...
	int query(String key,int counter) throws NoSuchAlgorithmException{
		int obj_id = service.calculate_sha1(key, ring_size);
		int value = -1; // i am not responsible for song
		if (iAmResponsibleForId(obj_id) ){
...
int delete (String key,int counter) throws NoSuchAlgorithmException{
		...
		if (!iAmResponsibleForId(obj_id) && counter < global_rep) {
			files.remove(key);
			answer = -3;
		}
		if (counter == 1){
  ...
  /* Questa funzione è eseguita da ogni thred nel Chord Ring
		...
		// Check if it is an answer
			 ...
			 System.out.println("Socket couldn't close during depart!");
				 ...
				 if (myid == hisSuccessorID ){
					 for (String key : files.keySet()) {
						 try {
					 if (service.calculate_sha1(key, ring_size) <= predecessor.getmyId()){
